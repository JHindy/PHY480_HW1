...
main ()
{
  ...
  int max_dimension = 400;		// dimension of the matrices and vectors 
...
for (int dimension = 1; dimension <= max_dimension; dimension++) {
  ...
  out << log(dimension) << "          " << fixed  
	<< setprecision(3) 
        << log((double) (end - start) / (double) CLOCKS_PER_SEC)
        << endl;
  ...
  // comment sdtarting here when running large matrices 
  /*for (int i = 0; i < dimension; i++)
    {
