...
const int N = 3;   // # of points in the data set
...
main (void)
{
  ...
  gsl_matrix *Jacobian_ptr = gsl_matrix_alloc (p,p);
  ...
  out << endl << endl << "#Initial data: " << endl;
  out <<  "#  t     y(t)     sigma(t)  " << endl;
  ...
  gsl_multifit_covar (Jacobian_ptr, 0.0, covariance_ptr);
