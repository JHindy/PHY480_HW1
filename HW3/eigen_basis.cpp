...
//
//
// Homework 3, question 3: From the plot we see that for the coulomb potential will converge to zero
// at large r for all different b_ho and dimmensions. When we have a low b_ho value, say 1, we see 
// that there is one large peak before it tapers off to zero. The dimmension seems to slightly alter 
// the severity of these peaks. When we increase b_ho to 10 we see much less of a peak. We can also
// now clearly see the effect that changing the dimmension has on the plots. Higher dimmensions mean
// slightly larger peaks. The effect does not seem to be as prominante as it is with b_ho however.
//
//
//
//
...
main ()
{
  ...
  double b_ho = 10;			// ho length parameter
  int dimension = 20;		// dimension of the matrices and vectors  
  //cout << "Enter the oscillator parameter b: ";
  //cin >> b_ho;


  //out << "b value                          ground state eigenvalue" << endl;
  ...
  ofstream out ("eigen_basis_2010.dat");
  double r = 0.1;
  double rend = 10;
  double dr = 0.1;
  out << "r    u(r)" << endl;
  //for (int i = 0; i < 1; i++)
    //{
      ...
      // }
  gsl_matrix_free (Eigvec_ptr);  // free the space used by the vector and matrices  and workspace 
