...
fit_title = sprintf("%-+4.1f*x %-+4.1f",a2,b2)

f3(x) = a3*x + b3
fit [-2:-1] f3(x) "derivative_test.dat" using ($1):($4) via a3,b3
fit_title = sprintf("%-+4.1f*x %-+4.1f",a3,b3)

f4(x) = a4*x +b4
fit [-1:0] f4(x) "derivative_test.dat" using ($1):($5) via a4,b4
fit_title = sprintf("%-+4.1f*x %-+4.1f",a4,b4)
...
plot "derivative_test.dat" using ($1):($2) title 'forward difference', \
     a1*x + b1 title fit_title, "derivative_test.dat" using ($1):($3) title 'central difference', \
     a2*x + b2 title fit_title, "derivative_test.dat" using ($1):($4) title 'extrapolated central difference', \
     a3*x + b3 title fit_title, "derivative_test.dat" using ($1):($5) title '2 extrapolated central difference',\
     a4*x + b4 title fit_title
