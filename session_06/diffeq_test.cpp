...
main (void)
{
  ...
  ofstream out ("diffeq_test_3.dat");	// open the output file 
  ...
  double hmin = 0.;		// starting t value 
  double hmax = 1.;		// last t value 
  double t = 1;		// initialize mesh spacing 
  ...
  out << "#      h           y_euler(t)         y_rk4(t)        y_exact(t) \n";
  out << scientific << setprecision (16)
    << hmin << "  "
    << y_euler[0] << "  "
    << y_rk4[0] << "  " << exact_answer (t, params_ptr) << endl;



  for (double h = hmin; h <= hmax; h += 0.01)
  {
